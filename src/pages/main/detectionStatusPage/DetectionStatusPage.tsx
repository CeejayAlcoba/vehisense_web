...
const DetectionStatusPage: React.FC = () => {
  ...
  const fetchStatus = async () => {
    try {
      ...
      // Conditions: 1) Status actually changed, 2) Not in cooldown, 3) Not initial load
      if (status && 
        ...
        const statusChange: Record<StatusType, { message: string; type: 'success' | 'error' | 'warning' | 'info'; icon: React.ReactNode }> = {
          running: { message: 'System Online', type: 'success', icon: <CheckCircleOutlined /> },
          error: { message: 'System Error', type: 'error', icon: <CloseCircleOutlined /> },
          stopped: { message: 'System Stopped', type: 'warning', icon: <WarningOutlined /> },
          starting: { message: 'System Starting', type: 'info', icon: <SyncOutlined /> },
        };
      ...
      // Only show error notification if not already shown recently
      if (!notificationCooldown.current) {
        api.error({
          message: 'Connection Error',
          description: 'Failed to fetch detection system status',
          placement: 'topRight',
          duration: 4.5
        });
        ...
        setTimeout(() => {
          notificationCooldown.current = false;
        }, 30000); // 30 second cooldown for errors
      }
    }
  };
  ...
  if (loading) {
    return (
      <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100vh' }}>
        <Spin size="large" tip="Loading detection system status..." />
      </div>
    );
  }
    ...
    <div style={{ padding: '24px', background: '#f0f2f5', minHeight: '100vh' }}>
      ...
      <Row justify="space-between" align="middle" style={{ marginBottom: 24 }}>
        <Col>
          <Title level={2} style={{ margin: 0 }}>
            <CameraOutlined /> Vehicle Detection System
          </Title>
          <Text type="secondary">
      ...
      {status && (
        <Alert
          message={
            <Space>
              <Badge status={badgeConfig.status} />
              <strong>System Status: {badgeConfig.text.toUpperCase()}</strong>
            </Space>
          }
          ...
          type={
            ...
            status.status === "starting" ? "info" : "warning"
          }
          showIcon
          style={{ marginBottom: 24, fontSize: 16 }}
          icon={
            status.status === "running" ? <CheckCircleOutlined /> :
            status.status === "error" ? <CloseCircleOutlined /> :
            status.status === "starting" ? <SyncOutlined spin /> :
            <WarningOutlined />
          }
        />
      )}
      ...
      <Row gutter={[16, 16]} style={{ marginBottom: 24 }}>
        <Col xs={24} sm={12} lg={6}>
          <Card>
            <Statistic
              title="Camera Status"
              value={status?.cameraConnected ? "Connected" : "Disconnected"}
              valueStyle={{ 
                color: status?.cameraConnected ? '#3f8600' : '#cf1322',
                fontSize: 20
              }}
        ...
        <Col xs={24} sm={12} lg={6}>
          <Card>
            <Statistic
              title="System Uptime"
              value={status ? formatUptime(status.uptimeSeconds) : "0h 0m 0s"}
      ...
      {/* Last Detection Card */}
      {status?.lastPlate && (
        <Card title="Last Detected Vehicle" style={{ marginBottom: 24 }}>
          <Row gutter={16} align="middle">
            ...
            <Col>
              <Text type="secondary">
                Detected: {dayjs(status.timestamp).format('YYYY-MM-DD HH:mm:ss')}
              </Text>
            </Col>
          </Row>
        </Card>
      ...
      {/* System Details Card */}
      <Card title="System Details" style={{ marginBottom: 24 }}>
        <Row gutter={[16, 16]}>
          <Col span={12}>
          ...
          <Col span={12}>
            ...
            <br />
            <Badge 
              status={status?.cameraConnected ? "success" : "error"} 
              text={status?.cameraConnected ? "Connected" : "Disconnected"} 
            />
          </Col>
          <Col span={12}>
            <Text strong>Last Update:</Text>
            <br />
            <Text>{status ? dayjs(status.timestamp).format('YYYY-MM-DD HH:mm:ss') : 'N/A'}</Text>
          </Col>
          <Col span={12}>
            <Text strong>Message:</Text>
            <br />
            <Text>{status?.message || 'No message'}</Text>
          </Col>
        </Row>
      </Card>
      ...
      <Card title="Recent System Logs">
        <Table
          columns={columns}
          dataSource={logs}
          rowKey={(record) => record.id || `log-${Math.random()}`}
          pagination={{ 
            pageSize: 20,
...
export default DetectionStatusPage;