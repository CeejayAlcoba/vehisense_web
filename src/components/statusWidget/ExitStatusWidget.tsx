...
const ExitStatusWidget: React.FC = () => {
  ...
  useEffect(() => {
    const fetchStatus = async () => {
      try {
        ...
        const runningStatus = await _detectionStatusService.isExitSystemRunning();
  ...
  return (
    <Card loading={loading} hoverable style={{ height: '100%', borderLeft: '4px solid #ff4d4f' }}>
      <Space direction="vertical" style={{ width: '100%' }} size="middle">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <Space>
            <LogoutOutlined style={{ fontSize: 24, color: '#ff4d4f' }} />
            <Text strong>EXIT System</Text>
          </Space>
          <Badge status={getBadgeStatus()} text={getStatusText()} />
        </div>
        <Statistic title="EXITS Today" value={status?.detectionsToday || 0} prefix={<LogoutOutlined />} valueStyle={{ color: '#ff4d4f', fontSize: 32 }} />
        <div style={{ borderTop: '1px solid #f0f0f0', paddingTop: 12 }}>
          <Space direction="vertical" style={{ width: '100%' }} size="small">
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <Text type="secondary">Camera:</Text>
              <Text strong style={{ color: status?.cameraConnected ? '#3f8600' : '#cf1322' }}>{status?.cameraConnected ? "Connected" : "Disconnected"}</Text>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <Text type="secondary">Vehicles Inside:</Text>
              <Text strong>{status?.activeVehicles || 0}</Text>
            </div>
            {status?.lastPlate && (
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <Text type="secondary">Last Exit:</Text>
...
export default ExitStatusWidget;
